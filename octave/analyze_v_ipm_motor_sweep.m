%% analyze_v_ipm_motor_sweep.m - V-IPM theta sweep + FEMM analyze
% 각 기계각 theta마다 로터 형상과 자화 방향을 함께 회전시켜 해석한다.
...
output_dir = fullfile(pwd, 'femm_output_v_ipm_sweep');
...
grid on;
title('V-IPM Torque vs Mechanical Angle');
...
grid on;
title('V-IPM F_x vs Mechanical Angle');
xlabel('\theta_{mech} [deg]');
ylabel('F_x');
...
grid on;
title('V-IPM F_y vs Mechanical Angle');
xlabel('\theta_{mech} [deg]');
ylabel('F_y');
